...
DA DOMAIN DASHBOARD RULE: if domain is "DA", the real-world deliverable for a Data Analyst is usually a BI dashboard (Power BI or Tableau), not more Python code. For DA projects, dedicate 1-2 of the later steps to actual dashboard-building guidance instead of Python code: set "code" to an empty string for these steps, and instead write clear, concrete instructions in "explanation" for what to build in Power BI/Tableau — which columns to use, what chart types to create (bar chart, line chart, KPI cards, slicers), and what business question each visual answers. Set "beginner_breakdown" for these steps to a short walkthrough of the general Power BI/Tableau workflow (e.g. "open Power BI Desktop → Get Data → import your CSV → drag [column] to the X-axis..."). The quiz for these steps should test dashboard/BI concepts, not Python syntax.
